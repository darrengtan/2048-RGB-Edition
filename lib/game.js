(function () {
  ...
  Game.prototype.checkGameEnd = function () {
    if (this.over()) {
      $('.end-message-background').removeClass("none");
      $('.end-message-content').removeClass("none");
      $('div.end-message-content .message').html(this.won ?
        "You Win!" : "Game Over");
    }
  };
  ...
  // convert arrow direction to vector
  ...
  Game.prototype.mergeTiles = function (tile, obstacle) {
    var game = this;
    var $tile = $(".tile.val-" + tile.val + ".pos-" + tile.pos[0] + "-" +
      tile.pos[1]);
    var $obstacle = $(".tile.val-" + obstacle.val + ".pos-" + obstacle.pos[0] +
      "-" + obstacle.pos[1]);
    game.moveTile(tile, obstacle.pos);
    tile.merged = true;
    game.updateScore(tile.val * 2);

    // css animation and delete obstacle tile from browser
  ...
  Game.prototype.moveTile = function (tile, pos) {
    // set transition to false after animation is done
  ...
  // allow merging for upcoming turn
  ...
  // board full but tiles can be merged still
  ...
  // travel in opposite direction of movement
  ...
  // add points of merged tile value
